...
export default function ResetPasswordPage() {
  ...
  return <AuthLayout>{done ? <><h1>Password updated.</h1><p className="rm-auth-success">Your account is secure again.</p><a className="rm-auth-primary-btn" href="/app">Open RoomMate</a></> : <><h1>Choose a new password.</h1><p className="rm-auth-sub">Make it memorable, but hard to guess.</p><form onSubmit={submit}><label className="rm-auth-label" htmlFor="new-password">New password</label><input id="new-password" className="rm-auth-field" type="password" autoComplete="new-password" minLength="8" placeholder="New password" value={password} onChange={(e) => setPassword(e.target.value)} required /><label className="rm-auth-label" htmlFor="confirm-password">Repeat password</label><input id="confirm-password" className="rm-auth-field" type="password" autoComplete="new-password" minLength="8" placeholder="Repeat password" value={confirm} onChange={(e) => setConfirm(e.target.value)} required />{error && <p className="rm-auth-error" role="alert">{error}</p>}<button className="rm-auth-primary-btn" type="submit" disabled={busy}>{busy ? 'Updating…' : 'Update password'}</button></form></>}</AuthLayout>;
}
