...
export default function BalancesPage({ balanceData = {}, members, onSettle }) {
  ...
  return (
    <div className="rm-animate-in max-w-4xl space-y-4">
      <div className="grid md:grid-cols-[1.2fr_.8fr] gap-4">
        ...
        <SectionCard>
          <div className="flex items-center gap-3 mb-3"><div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: '#F1EDFF' }}><Sparkles size={17} color={C.chores} /></div><div><p className="text-sm font-semibold" style={{ color: C.text }}>Simplify debts</p><p className="text-xs rm-text-secondary">Fewer payments, same totals.</p></div></div>
          <button type="button" role="switch" aria-checked={showSimplified} onClick={() => setShowSimplified((value) => !value)} className="flex items-center justify-between w-full text-left min-h-11"><span className="text-sm" style={{ color: C.text }}>{showSimplified ? 'Showing suggested payments' : 'Showing original pair balances'}</span><span className={`toggle-pill ${showSimplified ? 'on' : ''}`}><span /></span></button>
